...
export function ProfileStats({ user }: { user: User }) {
  ...
  // Real history (authed only; grades against stored results). 30-day window:
  // the day list shows the latest 7, the by-sport record uses all 30.
  const [history, setHistory] = useState<DayRec[]>([]);
  const [sportRecs, setSportRecs] = useState<SportRec[]>([]);
  useEffect(() => {
    if (!isAuthed || !authUserId || !slateKey) return;
    let active = true;
    (async () => {
      try {
        const dates = recentSlateKeys(30);
        ...
        if (active) {
          setHistory(dailyRecords(picks, results, slateKey, games).slice(0, 7));
          setSportRecs(leagueRecords(picks, results, slateKey, games));
        }
      } catch {
        /* leave empty */
      }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Record by sport (last 30 days) — e.g. your World Cup record */}
      {sportRecs.length > 0 && (
        <div>
          <SectionLabel>By sport</SectionLabel>
          <div className="space-y-1.5">
            {sportRecs.map((r) => {
              const losses = r.completed - r.correct;
              const pct =
                r.completed > 0
                  ? Math.round((r.correct / r.completed) * 100)
                  : null;
              const meta = safeMeta(r.league);
              return (
                <div
                  key={r.league}
                  className="flex items-center gap-3 rounded-xl border border-line bg-ink-850/60 px-3.5 py-2.5"
                >
                  <span
                    className="grid h-7 w-12 shrink-0 place-items-center rounded-lg text-[10px] font-extrabold"
                    style={{
                  ...
                  {pct === null ? (
                    ...
                    <>
                      ...
                      <span
                        className={cn(
                          "tnum w-11 text-right text-sm font-extrabold",
                          pct >= 50 ? "text-lime" : "text-ink-600"
                        )}
                      >
                        {pct}%
                      </span>
          ...
          <p className="mt-2 px-0.5 text-[11px] text-ink-600">
            Last 30 days. Wins–losses on completed games.
          </p>
        </div>
      )}
...
function safeMeta(league: string) {
  try {
    const m = sportMeta(league as League);
    if (m) return m;
  } catch {
    /* fall through */
  }
  return { label: league, abbr: league.slice(0, 4), accent: "#9AA0AB" };
}
